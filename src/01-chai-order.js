...
export function hasSpecialIngredient(order, ingredient) {
  // Your code here
  if( typeof order !== "string" || order.trim().length == 0) return false

  if( typeof ingredient !== "string" || ingredient.trim().length == 0) return false

  if(order.toLowerCase().includes(ingredient.toLocaleLowerCase())) return true
  else return false
}
